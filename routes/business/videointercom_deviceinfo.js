...
router.use('/deviceinfo', function(req, res, next) {
    ...
    seckeyPool.get(seckey, function (loginuser) {
        ...
        let tablename = constUtils.TABLE_P_VIDEOINTERCOM_DEVICEINFO;
        ...
        if (seckey != '' && seckey != undefined && seckey != null) {
            postgredb.getTbleColInfo(constUtils.TABLE_P_VIDEOINTERCOM_DEVICEINFO, json, uploadData);
        ...
        if(checkResult.status){
            seckeyPool.get(seckey, function (loginuser) {
                if (loginuser != null) {
                    ...
                    res.send('{"code":' + constUtils.WORK_QUERY_FAIL + ',"msg":"[可视开门系统设备的基本信息]无效seckey，操作不成功！"}  上传时间:' + moment().format('YYYY-MM-DD HH:mm:ss'));
                }
            });
        }else{
            res.send('{"code":' + constUtils.WORK_DATA_ERR + ',"msg":[可视开门系统对讲记录信息]数据不正确，上传失败！失败原因：'+JSON.stringify(checkResult.msg).replace(/\\/g,"")+'}  上传时间:' + moment().format('YYYY-MM-DD HH:mm:ss'));
        }
    }
});

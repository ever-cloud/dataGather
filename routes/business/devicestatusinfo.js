...
router.use('/devicestatusinfo', function(req, res, next) {
    ...
    if(seckey!='' && seckey!=undefined && seckey!=null) {
        seckeyPool.get(seckey, function (loginuser) {
            ...
            res.send('{"code":' + constUtils.WORK_UPLOAD_SUCCESS + ',"msg":"[设备使用状态信息]数据上传ActiveMq成功！"} 上传时间:' + moment().format('YYYY-MM-DD hh:mm:ss'));
            }else{
                res.send('{"code":' + constUtils.WORK_QUERY_FAIL + ',"msg":"[设备使用状态信息]无效seckey，操作不成功！"} 时间:' + moment().format('YYYY-MM-DD hh:mm:ss'));
            }
        });
    }

 ...
 *
 */
router.use('/caruserinfo', function(req, res, next) {
    ...
    seckeyPool.get(seckey, function (loginuser) {
        ...
        let tablename = constUtils.TABLE_P_PARKING_CARUSERINFO;
        ...
        if (seckey != '' && seckey != undefined && seckey != null) {
            postgredb.getTbleColInfo(constUtils.TABLE_P_PARKING_CARUSERINFO, json, uploadData);
        ...
        if(checkResult.status){
            seckeyPool.get(seckey, function (loginuser) {
                if (loginuser != null) {
                    ...
                    loginuser['tableName'] = constUtils.TABLE_P_PARKING_CARUSERINFO;
                    ...
                    res.send('{"code":' + constUtils.WORK_UPLOAD_SUCCESS + ',"msg":"[停车场系统车主信息]数据上传ActiveMq成功！"} 上传时间:' + moment().format('YYYY-MM-DD HH:mm:ss'));
                }else{
                    res.send('{"code":' + constUtils.WORK_QUERY_FAIL + ',"msg":"[停车场系统车主信息]无效seckey，操作不成功！"}  上传时间:' + moment().format('YYYY-MM-DD HH:mm:ss'));
                }
            });
        }else{
            res.send('{"code":' + constUtils.WORK_DATA_ERR + ',"msg":[停车场系统车主信息]数据不正确，上传失败！失败原因：'+JSON.stringify(checkResult.msg).replace(/\\/g,"")+'}  上传时间:' + moment().format('YYYY-MM-DD HH:mm:ss'));
        }
    }
});

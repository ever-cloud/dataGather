...
let initStatistics = require('./initStatistics');
...
let _stat=function(communityid,types,typearrays,cb){
    switch(types){
        case 'online'://统计系统在线
            ...
            postgre.excuteSql(systeminfoSql,[communityid],function (result){
                console.log('开始处理统计系统在线'+communityid+'初始的系统类型'+systemtypes);
                if(result.rowCount>0){
                    //有系统数据时候
                        ...
                        if(isexists===1){
                            ...
                            redis.hgetall('stat:c:'+communityid,(communitystats)=>{
                                Object.keys(communitystats).forEach(function (field,index) {
                                    ...
                                    let newstatinfo=JSON.parse(communitystats[field]);
                        ...
                        if(isexists===1){
                            ...
                            redis.hgetall('stat:c:'+communityid,(communitystats)=>{
                                Object.keys(communitystats).forEach(function (field,index) {
                                    ...
                                    if(oldstatinfo['online']!=undefined && parseInt(oldstatinfo['online'])!=0){
                                        calinfo[field]=-oldstatinfo['online'];
                                        oldstatinfo['online']=0;
                                        redis.hset('stat:c:'+communityid,field,JSON.stringify(oldstatinfo));
            ...
            if(typearrays instanceof Array && typearrays.length > 0){
                ...
                typearrays.forEach((stattype,dindex)=>{
                    ...
                    if(stattype=='elevator'){
                        realalarmsql='select t.communityid,count(t.communityid) as "alarm" FROM p_devicealarm t ,(SELECT cs.communityid,cs.deviceid,cs.systemid,cs.status FROM p_elevator_deviceinfo cs,t_community_system tcs where tcs.sid=\'7\' and tcs.communityid=cs.communityid and tcs.systemid=cs.systemid   and  cs.communityid=$1) gl where gl.communityid=t.communityid and gl.systemid=t.systemid and gl.deviceid=t.deviceid and  t.communityid=$1 and t.sid=\'7\' and t."datetime" BETWEEN CURRENT_DATE and CURRENT_DATE+1 group by t.communityid ';
                        alarmservicesql='select o.communityid,count(o.communityid) as "alarm" from (SELECT distinct t.communityid,t.deviceid FROM p_devicealarm t ,(SELECT cs.communityid,cs.deviceid,cs.systemid,cs.status FROM p_elevator_deviceinfo cs,t_community_system tcs where tcs.sid=\'7\' and tcs.communityid=cs.communityid and tcs.systemid=cs.systemid  and  cs.communityid=$1) gl where gl.communityid=t.communityid and gl.systemid=t.systemid and gl.deviceid=t.deviceid and  t.communityid=$1 and t.sid=\'7\' and t."datetime" BETWEEN CURRENT_DATE and CURRENT_DATE+1) o group by o.communityid';
                        ...
                        realalarmsql='select o.communityid,count(o.communityid) as "alarm" from p_devicealarm o ,(SELECT cs.communityid,cs.deviceid,cs.systemid,cs.status FROM p_videointercom_deviceinfo cs,t_community_system tcs where tcs.sid=\'2\' and tcs.communityid=cs.communityid and tcs.systemid=cs.systemid  and cs.communityid=$1) gl where gl.communityid=o.communityid and gl.systemid=o.systemid and gl.deviceid=o.deviceid and  o.communityid=$1 and o.sid=\'2\' and o."datetime" BETWEEN CURRENT_DATE and CURRENT_DATE+1 group by o.communityid';
                        alarmservicesql='select o.communityid,count(o.communityid) as "alarm" from(SELECT distinct t.communityid,t.deviceid FROM p_devicealarm t ,(SELECT cs.communityid,cs.deviceid,cs.systemid,cs.status FROM p_videointercom_deviceinfo cs,t_community_system tcs where tcs.sid=\'2\' and tcs.communityid=cs.communityid and tcs.systemid=cs.systemid  and cs.communityid=$1) gl where gl.communityid=t.communityid and gl.systemid=t.systemid and gl.deviceid=t.deviceid and  t.communityid=$1 and t.sid=\'2\' and t."datetime" BETWEEN CURRENT_DATE and CURRENT_DATE+1) o group by o.communityid';
                    }else if(stattype=='location'){
                        realalarmsql='select p.communityid,count(p.communityid) as "alarm" from p_personlocation_alarm p ,(SELECT cs.communityid,cs.cardid,cs.systemid FROM p_personlocation_givecard cs where  cs.communityid=$1) gl where gl.communityid=p.communityid and gl.systemid=p.systemid and gl.cardid=p.cardid and  p.communityid=$1 and p.datetime BETWEEN CURRENT_DATE and CURRENT_DATE+1  group by p.communityid';
                        alarmservicesql='select p.communityid,count(p.communityid) as "alarm" from p_personlocation_alarm p ,(SELECT cs.communityid,cs.cardid,cs.systemid FROM p_personlocation_givecard cs where  cs.communityid=$1) gl where gl.communityid=p.communityid and gl.systemid=p.systemid and gl.cardid=p.cardid and  p.communityid=$1 and p.alarmtype=\'1\'   group by p.communityid';
                    }else if(stattype=='gate'){
                        alarmservicesql='select t.communityid,count(t.communityid) as "alarm" from p_devicealarm t ,(SELECT cs.communityid,cs.deviceid,cs.systemid,cs.status FROM p_gate_deviceinfo cs,t_community_system tcs where tcs.sid=\'5\' and tcs.communityid=cs.communityid and tcs.systemid=cs.systemid and cs.communityid=$1) gl where gl.communityid=t.communityid and gl.systemid=t.systemid and gl.deviceid=t.deviceid  and  t.communityid=$1 and t.sid=\'5\'  group by t.communityid';
                    }
                    if(realalarmsql != ''){

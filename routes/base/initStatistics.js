...
function initStatistics(){
    ...
    this.createStatistics=()=>{
        ...
        postgre.excuteSql(deptidSql,[],function (result){
                    ...
                    redis.hget(constUtils.TABLE_P_SYSTEMINFO,data.communityid,(systemInfoList_data)=>{
                        if(systemInfoList_data !=null){
                            ...
                            systemInfoList.forEach((systeminfo,index)=>{
                                let communitySum = 0;
                                switch(systeminfo.sid)
                                {
                                    case '1'://视频监控
                                        if(systeminfo.status=='null' || systeminfo.status=='1'){
                                            communitySum=1;
                                            middleObj['stat:c:'+data.communityid].monitor.online=communitySum;
                                            ...
                                            redis.hget('deptdict:'+data.communityid,'regionid',(deptid)=>{
                                                    middleObj['stat:r:' + deptid].monitor.online += communitySum;
                                            ...
                                            redis.hget('deptdict:'+data.communityid,'groupid',(deptid)=>{
                                                middleObj['stat:g:' + deptid].monitor.online += communitySum;
                                            ...
                                            redis.hget('deptdict:'+data.communityid,'haierid',(deptid)=>{
                                                middleObj['stat:h:' + deptid].monitor.online += communitySum;
            ...
            //视频监控
            //设备数和故障数
            let monitorSql='SELECT t.communityid,sum(case when t.devicetype=\'camera\'  then 1 else 0 end) as "camerasum",sum(case when t.devicetype=\'camera\'  and t.status=\'2\' then 1 else 0 end) as "camerabug",sum(case when t.devicetype=\'dvr\'  then 1 else 0 end) as "dvrsum",sum(case when t.devicetype=\'dvr\' and t.status=\'2\' then 1 else 0 end) as "dvrbug" FROM p_videomonitor_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            //电梯监控
            //设备数和警报数和故障数
            let elevatorbugSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_elevator_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            postgre.excuteSql(elevatoralarmSql,[],function (result){// 实时监控
                if(result.rowCount>0){
                    result.rows.forEach(function(data){
            ...
            //可视对讲
            //设备数和警报数和故障数
            let intercombugSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_videointercom_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            //人员定位
            //设备数和警报数和故障数和发卡数
            let locationbugSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_personlocation_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            //gate
            //设备数和警报数和故障数
            let gatebugSql='SELECT t.communityid,count(id) as "sum",SUM(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_gate_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            //信息发布
            //设备数和故障数
            let infobugSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_infodiffusion_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            let intrusionbugSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_alarm_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            let parkbugSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_parking_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            let broadcastbugSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_broadcast_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
            ...
            //电子巡更
            //终端总数和已巡更次数和未巡更次数
            let patrolsumSql='SELECT t.communityid,count(id) as "sum",sum(case when t.status=\'2\' then 1 else 0 end) as "bug" FROM p_patrol_deviceinfo t where t.status<>\'3\' group by t.communityid order by t.communityid';
                    ...
                    redis.hgetall(key,(data)=>{
                        ...
                        if(communityStatInfo.length==keys.length && keyindex==keys.length-1){
                            ...
                            if(j==publishRole.length){
                                // console.log('发布被调用了！！！'+j+':'+keyindex+':'+key);
